...
export const ShareOptions = (attributes) => {
	...
	const modeOptions = [
		{
			label: __('Link to social networks', 'mustra-designs'),
			icon: icons.link,
			value: 'links',
		},
		{
			label: __('Share the current URL', 'mustra-designs'),
			subtitle: __('Unavailable for some networks', 'mustra-designs'),
			icon: icons.share,
			value: 'share',
		},
	];

	const pickerSectionTitle = shareMode ? __('Share targets', 'mustra-designs') : __('Social networks', 'mustra-designs');
	...
	return (
		...
		>
			<OptionSelect
				aria-label={__('Share mode', 'mustra-designs')}
				value={shareMode}
				options={modeOptions}
				onChange={(value) =>
					setAttributes({
