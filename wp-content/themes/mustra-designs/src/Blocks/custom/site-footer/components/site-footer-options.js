		...
		<>
			...
			>
				{(item) => {
					...
					return (
						<RepeaterItem
							label={header?.length > 0 ? header : __('New section', 'mustra-designs')}
							subtitle={items.length > 0 && sprintf(_n('%d link', '%d links', items.length, 'mustra-designs'), items.length)}
						>
							<Repeater
								icon={icons.link}
								label={__('Links', 'mustra-designs')}
							...
							>
								{(item) => {
									...
									return (
										<RepeaterItem
											label={text?.length > 0 ? text : __('New link', 'mustra-designs')}
